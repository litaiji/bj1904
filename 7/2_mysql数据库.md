...
- 流程控制函数
  ...
  - if(expr1,expr2,expr3) 如果expr1为真则返回expr2,否则返回expr3
...
- 修改密码
  ...
  mysql5.7之后
  use mysql
  update user set authentication_string = password('2333') where user='root'
  ~~~
...
一般经常出现在where、order by、group by having后面的字段可以建立索引
...
- 备份
  ...
  #备份一个或多个数据库
  mysqldump –uroot –p 数据库名1 数据库名2.. > ~/备份文件名.sql;
